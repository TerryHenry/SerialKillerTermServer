...
# Meant to be run directly over SSH (port 22) as root -- NOT reachable through the app
# itself, since the whole point is to still work if a bad update left the app unable to
# serve requests at all.
#
#   sudo bash /opt/terminalserver/provisioning/rollback-update.sh
#
set -euo pipefail
...
echo "Stopping terminalserver.service..."
systemctl stop terminalserver.service
...
echo "Starting terminalserver.service..."
systemctl start terminalserver.service
sleep 2
systemctl --no-pager status terminalserver.service || true
