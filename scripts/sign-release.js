 ...
 * private key, producing the detached signature asset (terminalserver-app.tar.gz.sig)
 * that lib/selfUpdate.js verifies before an appliance will self-apply an update.
 *
 ...
 *
 * The private key never lives in this repo and this script never asks for one by a
 * default path -- always pass it explicitly, from wherever you actually keep it
 * (password manager export, hardware key, offline USB), so there's no default
 ...
 *
 * Example:
 *   node scripts/sign-release.js build/terminalserver-app.tar.gz.sha256 \
 *     /path/to/release-signing-key.PRIVATE.pem \
 *     build/terminalserver-app.tar.gz.sig
 */
