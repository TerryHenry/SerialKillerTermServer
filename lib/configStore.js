...
const defaults = {
  ...
  // Off by default -- unlike the other fleet channels (admin-UI proxy, one-shot device
  // listing, port streaming), this hands the hub a real interactive shell on this box,
  // running as this app's own unprivileged service account. No sudo, no root, but still
  // meaningfully more than anything else reachable over the tunnel, so it shouldn't come
  // up on a freshly-enrolled box without an admin deliberately turning it on -- see the
  // Fleet panel.
  remoteShell: {
    enabled: false
  },
  // Off by default, same reasoning as webTerminal/remoteShell -- this box likely has no
  // UPS attached at all, and NUT (nut-server + a driver) isn't installed until an admin
  // actually configures one. name is the NUT UPS identifier used throughout ups.conf/
  // upsd.users/upsmon.conf (upsc queries it as "<name>@localhost"); driver/port select
  // which NUT driver talks to the actual hardware -- see lib/upsMonitor.js.
  ups: {
    enabled: false,
    name: 'ups',
    driver: 'usbhid-ups',
    port: 'auto',
    // Only meaningful for specific drivers -- community for snmp-ups (its SNMPv1
    // shared secret, not a real username/password), identifier for cyclades-pm10 (the
    // powerman device/node name this app generates locally -- NUT's powerman-pdu driver
    // itself doesn't use it). Persisted regardless of driver so switching drivers and
    // back doesn't lose what was typed.
    community: '',
    identifier: '',
    // cyclades-pm10 only: the PM10's own serial login. Deliberately NOT paired with a
    // persisted password field -- the password is forwarded to the privileged helper at
    // configure time and only ever lives on disk root-owned under /etc/powerman, same as
    // every other secret this app hands off rather than keeps in its own config.json.
    username: ''
  },
  tftp: {
    port: 69,
...
function mergeWithDefaults(raw) {
  ...
  return {
    ...
    remoteShell: { ...defaults.remoteShell, ...raw.remoteShell },
    ups: { ...defaults.ups, ...raw.ups },
...
function updateRemoteShell(partial) {
  state.remoteShell = { ...state.remoteShell, ...partial };
  persist();
  return state.remoteShell;
}

function updateUps(partial) {
  state.ups = { ...state.ups, ...partial };
  persist();
  return state.ups;
}
...
/** Applies a fleet "sync-users" command: replaces this box's ENTIRE console-user table with the
 * hub's list -- accounts created locally are removed too, the same as admin sync replaces the
 * whole admin table -- so the box's logins match the hub's exactly. Like admin sync it refuses
 * an empty or malformed list (a sync that happens to match nobody must not wipe every login).
 * Takes already-hashed {username, passwordSalt, passwordHash, permission, captureEnabled}
 * entries. Synced accounts are password-only, with no 2FA, no public key and no default port;
 * those start fresh, as they do for synced admins. */
function replaceUsers(entries) {
  if (!Array.isArray(entries) || entries.length === 0) {
    throw new Error('refusing to replace users with an empty or invalid list');
  }
  const seen = new Set();
  const applied = entries.map((e) => {
    ...
    if (seen.has(e.username)) throw new Error(`the synced list names "${e.username}" twice`);
    seen.add(e.username);
    return {
      id: crypto.randomUUID(),
      username: e.username,
  ...
  const removed = state.users.filter((u) => !seen.has(u.username)).map((u) => u.username);
  state.users = applied;
  persist();
