...
const defaults = {
  ...
  fleet: {
    ...
    hubApiPort: 8443,
    // Blank means the tunnel accepts the hub's host key unverified (today's behavior,
    // kept for compatibility with already-enrolled sites) -- paste the value from the
    // hub's own System tab here to pin it and close that gap.
    hubHostKeyFingerprint: '',
    // Blank means outbound HTTPS calls to the hub (heartbeat, backup push, enrollment)
    // trust whatever certificate it presents, same trust-on-first-use posture as an
    // unpinned hubHostKeyFingerprint above -- paste the hub's TLS fingerprint (its own
    // System tab) here to pin it via lib/pinnedHttps.js and close that gap too.
    hubTlsFingerprint: ''
  },
  users: [],
  ports: []
