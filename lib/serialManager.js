...
class SerialManager extends EventEmitter {
  constructor() {
    ...
    // Cumulative per-port byte counters, kept separately from openPorts so a count
    // survives a port closing and reopening (last session leaving, a new one
    // attaching) rather than resetting every time refCount hits zero.
    this.portStats = new Map(); // portId -> { rxBytes, txBytes, lastActivity }
  }

  bumpStats(portId, direction, length) {
    let stats = this.portStats.get(portId);
    if (!stats) {
      stats = { rxBytes: 0, txBytes: 0, lastActivity: null };
      this.portStats.set(portId, stats);
    }
    if (direction === 'rx') stats.rxBytes += length;
    else stats.txBytes += length;
    stats.lastActivity = Date.now();
  }

  /** Cumulative rx/tx byte counts for a configured port, since this process started. */
  getStats(portId) {
    return this.portStats.get(portId) || { rxBytes: 0, txBytes: 0, lastActivity: null };
  }
  ...
  async open(portConfig, sessionId) {
    ...
    if (entry) {
      ...
      // Installed once per physical open, independent of how many sessions attach
      // afterward, so counters and the debug tap see every byte exactly once rather
      // than once per attached session.
      serialPort.on('data', (chunk) => {
        this.bumpStats(portConfig.id, 'rx', chunk.length);
        this.emit('traffic', portConfig.id, 'rx', chunk);
      });
      const rawWrite = serialPort.write.bind(serialPort);
      serialPort.write = (chunk, ...args) => {
        this.bumpStats(portConfig.id, 'tx', Buffer.byteLength(chunk));
        this.emit('traffic', portConfig.id, 'tx', Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
