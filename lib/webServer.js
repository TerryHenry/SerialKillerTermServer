...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  app.post('/api/fleet', requireFullAuth, async (req, res) => {
    const { mode, hubHost, hubPort, hubApiPort, hubHostKeyFingerprint, hubTlsFingerprint, allowForwarding } = req.body || {};
    ...
    const fleet = configStore.updateFleet({
      ...
      allowForwarding: allowForwarding === undefined ? configStore.getConfig().fleet.allowForwarding !== false : !!allowForwarding
    });
    tunnelClient.stop();
    fleetHeartbeat.stop();
    if (mode === 'managed') {
    ...
    try {
      ...
      if (apiPort === null) {
        return res.status(502).json({
          error: `tunneled enrollment failed (${tunnelErr.message}) and no API port is configured to fall back to`
        });
      }
      onLog(`[${logTimestamp()}] [fleet] tunneled enrollment failed (${tunnelErr.message}) -- falling back to a direct connection`);
