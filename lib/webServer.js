...
const upsMonitor = require('./upsMonitor');
...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  // ---------- UPS / power monitoring (NUT, via the privileged helper) ----------
  app.get('/api/ups', requireFullAuth, async (req, res) => {
    res.json(await upsMonitor.getFullStatus());
  });

  app.post('/api/ups', requireFullAuth, async (req, res) => {
    const { enabled, name, driver, port, community, identifier } = req.body || {};
    try {
      const config = await upsMonitor.setConfig({ enabled: !!enabled, name, driver, port, community, identifier });
      audit(req, `${enabled ? `configured UPS monitoring (driver ${driver}, port ${port})` : 'disabled UPS monitoring'}`);
      res.json(config);
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
