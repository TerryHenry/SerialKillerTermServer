...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  app.post('/api/fleet', requireFullAuth, async (req, res) => {
    ...
    const fleet = configStore.updateFleet({ mode, hubHost: (hubHost || '').trim(), hubPort: port, hubApiPort: apiPort });
    tunnelClient.stop();
    fleetHeartbeat.stop();
    if (mode === 'managed') {
      tunnelClient.start();
