...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  // Talks to the hub's own admin API port (hubApiPort), separate from the tunnel/SSH
  // listener (hubPort) that POST /api/fleet above configures.
  app.post('/api/fleet/enroll', requireFullAuth, async (req, res) => {
    ...
    const { hubTlsFingerprint } = configStore.getConfig().fleet;
