...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  app.post('/api/fleet', requireFullAuth, async (req, res) => {
    const { mode, hubHost, hubPort, hubApiPort, hubHostKeyFingerprint } = req.body || {};
  ...
  app.post('/api/fleet/enroll', requireFullAuth, async (req, res) => {
    ...
    let hubResponse;
    try {
      hubResponse = await new Promise((resolve, reject) => {
          ...
          (r) => {
            ...
            r.on('end', () => {
              ...
              try {
                ...
                return reject(new Error('hub returned invalid JSON'));
              }
              if (r.statusCode < 200 || r.statusCode >= 300) return reject(new Error(parsed.error || `HTTP ${r.statusCode}`));
              resolve(parsed);
            });
          }
        );
