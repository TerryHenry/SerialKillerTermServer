...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  app.post('/api/ports/power-test', requireFullAuth, async (req, res) => {
    const { deviceId, localKey, ip, version } = req.body || {};
    if (!deviceId || !localKey || !ip) {
      return res.status(400).json({ error: 'device ID, local key, and IP are all required' });
    }
    try {
      const result = await powerControl.testConnection({ deviceId, localKey, ip, version });
      res.json({ ok: true, on: result.on });
    } catch (e) {
      res.status(502).json({ error: e.message || 'could not reach the plug' });
    }
  });
