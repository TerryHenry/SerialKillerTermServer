...
function createWebServer(sshServer, tftpServer, hostKeyPublic) {
  ...
  function audit(req, message) {
    ...
    onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] "${who}" ${message}`);
  }
  ...
  app.post('/api/login', (req, res) => {
    ...
    onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] login failed for "${username}" from ${ip}`);
  ...
  app.post('/api/logout', (req, res) => {
    ...
    req.session.destroy(() => {
      if (who) onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] "${who}" logged out`);
      res.json({ ok: true });
    });
  });
  ...
  app.post('/api/terminal/login', (req, res) => {
    ...
    if (!validPassword) {
      ...
      onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] web terminal login failed for "${username}" from ${ip}`);
    ...
    onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] "${user.username}" logged into the web terminal from ${ip}`);
  ...
  app.post('/api/terminal/logout', (req, res) => {
    ...
    if (who) onLog(`[${new Date().toLocaleTimeString()}] [AUDIT] "${who}" logged out of the web terminal`);
    res.json({ ok: true });
  });
  ...
  app.get('/api/network', requireFullAuth, async (req, res) => {
    const [interfaces, publicIp, wifiRadio] = await Promise.all([
      networkInfo.getInterfaces(),
      networkInfo.getPublicIp(),
      networkInfo.getWifiRadioState()
    ]);
    res.json({ interfaces, publicIp, wifiRadio });
  });
  ...
  function attachTerminalSocket(httpsServer) {
    webTerminal.attach(httpsServer, sessionMiddleware, onLog);
  }
