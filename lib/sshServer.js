...
const ESCAPE_BYTE = 0x1d; // Ctrl+] — detach back to the port menu
const POWER_BYTE = 0x10; // Ctrl+P — power control menu for this port's plug, if any
      ...
      .then(
        ({ serialPort, release, canWrite: portCanWrite }) =>
          new Promise((resolve) => {
            ...
            const hasPlug = powerControl.hasPlug(portConfig);
            ...
            if (hasPlug) stream.write('Press Ctrl+P for power control (on/off/cycle) of this port\'s plug.\r\n');
            if (!canWrite()) stream.write('This session is read-only; your input will not be sent to the port.\r\n');
            stream.write('\r\n');

            let done = false;
            let awaitingPowerChoice = false;
            ...
            const runPowerAction = async (choice) => {
              const actions = {
                o: { label: 'Powering on', run: () => powerControl.setPower(portConfig, true) },
                f: { label: 'Powering off', run: () => powerControl.setPower(portConfig, false) },
                c: { label: 'Power cycling', run: () => powerControl.powerCycle(portConfig) }
              };
              const action = actions[choice];
              if (!action) {
                stream.write('[Power control: cancelled]\r\n');
                return;
              }
              stream.write(`[${action.label} "${portConfig.label}"'s plug...]\r\n`);
              try {
                await action.run();
                stream.write('[Done]\r\n');
                this.log(`"${sessionRecord.username}" ${action.label.toLowerCase()} plug for "${portConfig.label}"`);
              } catch (err) {
                stream.write(`[Power control failed: ${err.message}]\r\n`);
                this.log(`Power control failed for "${sessionRecord.username}" on "${portConfig.label}": ${err.message}`);
              }
            };

            // Byte-by-byte rather than a single indexOf scan: Ctrl+P isn't session-ending like
            // Ctrl+], it just diverts the *next* byte to the power menu instead of the serial
            // port, so surrounding bytes in the same chunk still need normal handling.
            const onStreamData = (data) => {
              let forwardStart = 0;
              for (let i = 0; i < data.length; i++) {
                const byte = data[i];

                if (awaitingPowerChoice) {
                  if (i > forwardStart && canWrite()) serialPort.write(data.slice(forwardStart, i));
                  forwardStart = i + 1;
                  awaitingPowerChoice = false;
                  runPowerAction(String.fromCharCode(byte).toLowerCase());
                  continue;
                }
                ...
                if (byte === POWER_BYTE) {
                  ...
                  forwardStart = i + 1;
                  if (!canWrite()) {
                    stream.write('\r\n[Power control requires write access]\r\n');
                  } else if (!hasPlug) {
                    stream.write('\r\n[No power plug configured for this port]\r\n');
                  } else {
                    stream.write('\r\n[Power: (o)n  (f)off  (c)ycle  any other key = cancel] ');
                    awaitingPowerChoice = true;
                  }
                  continue;
                }
              }
              if (data.length > forwardStart && canWrite()) serialPort.write(data.slice(forwardStart));
            };
            const onSerialClose = () => {
