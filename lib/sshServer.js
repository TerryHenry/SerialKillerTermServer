...
class SSHServerManager extends EventEmitter {
  ...
  handleClient(client) {
    let authedUser = null;
    let pendingTotpUser = null; // primary factor (password/publickey) already verified, awaiting the 6-digit code
    const sock = client._sock;
    const remoteInfo = sock ? `${sock.remoteAddress}:${sock.remotePort}` : 'unknown';
    // Throttling needs the bare IP, not ip:port -- the port is a fresh ephemeral value on
    // every single connection attempt, so keying on remoteInfo would never accumulate
    // failures across attempts and the lockout would never actually trigger.
    const remoteIp = sock ? sock.remoteAddress : 'unknown';

    // Primary auth (password/publickey) succeeded for this user -- either finish the
    // login outright, or, if they have 2FA on, partially-accept and require
    // keyboard-interactive next (the standard SSH multi-factor pattern: reject() with
    // isPartial=true tells the client this method succeeded but more is required).
    const finishPrimaryAuth = (ctx, user) => {
      if (user.totpEnabled) {
    ...
    client.on('authentication', (ctx) => {
      ...
      if (ctx.method === 'password') {
        if (
          ...
          return finishPrimaryAuth(ctx, user);
        }
        return ctx.reject(['password', 'publickey']);
      }
      ...
      if (ctx.method === 'keyboard-interactive') {
        // Only reachable after a primary factor already partially-succeeded for this
        // exact user -- a client can't jump straight to this method.
        if (!pendingTotpUser || pendingTotpUser.username !== ctx.username) {
          return ctx.reject(['password', 'publickey']);
        }
        const totpUser = pendingTotpUser;
        const throttleKey = `ssh-2fa:${totpUser.username}`;
        if (loginThrottle.isLocked(remoteIp, throttleKey)) {
          const secs = loginThrottle.remainingLockSeconds(remoteIp, throttleKey);
          this.log(`2FA locked out for "${totpUser.username}" from ${remoteInfo} (${secs}s remaining)`);
          return ctx.reject([]);
        }
        ctx.prompt([{ prompt: 'Verification code: ', echo: false }], 'Two-Factor Authentication', '', (responses) => {
          const token = (responses && responses[0]) || '';
          if (totp.verifyToken(totpUser.totpSecret, token)) {
            loginThrottle.recordSuccess(remoteIp, throttleKey);
            pendingTotpUser = null;
            authedUser = totpUser;
            configStore.recordUserLogin(totpUser.id);
            this.log(`"${totpUser.username}" completed 2FA from ${remoteInfo}`);
            return ctx.accept();
          }
          loginThrottle.recordFailure(remoteIp, throttleKey);
          this.log(`Wrong 2FA code for "${totpUser.username}" from ${remoteInfo}`);
          return ctx.reject(['keyboard-interactive'], true);
        });
        return;
      }
  ...
  async runMenuLoop(sessionRecord, stream) {
    while (!stream.destroyed) {
      ...
      ports.forEach((p, i) => {
        ...
        stream.write(`  [${i + 1}] ${p.label}  (${p.baudRate} baud)${tag}\r\n`);
      });
      ...
      .then(
        ({ serialPort, release, canWrite: portCanWrite }) =>
          new Promise((resolve) => {
            ...
            const capture = sessionCapture.startCapture(portConfig, {
              username: sessionRecord.username,
              method: 'ssh',
              userCaptureEnabled: sessionRecord.captureEnabled
            });
            ...
            const onStreamData = (data) => {
              ...
              if (escIdx !== -1) {
                if (escIdx > 0 && canWrite()) {
                  serialPort.write(data.slice(0, escIdx));
                  capture?.write(data.slice(0, escIdx));
                }
                stream.write('\r\n[Detached from port]\r\n');
                finish(true);
                return;
