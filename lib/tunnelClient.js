...
class TunnelClient extends EventEmitter {
  ...
  openHubApiChannel() {
    return new Promise((resolve, reject) => {
      ...
      // forwardOut's callback has no timeout of its own -- confirmed in production that
      // it can hang forever with neither resolving nor rejecting when the underlying
      // connection has already gone stale on the far end (the hub closed it, but this
      // side's ssh2 Client didn't notice -- see forceReconnect() below). Without this,
      // every heartbeat tick just adds another silently-hanging promise instead of ever
      // surfacing an error fleetHeartbeat.js could act on.
      let settled = false;
      const timer = setTimeout(() => {
        if (settled) return;
        settled = true;
        reject(new Error('timed out opening a channel over the tunnel'));
      }, 10000);
  ...
  /** Escape hatch for fleetHeartbeat.js's watchdog: forces a fresh connection when the
   * tunnel still reports connected but has stopped actually working. Known to happen
   * when ssh2's own client-side keepalive goes silent -- if its retry count is exceeded
   * at a moment the socket has already gone half-closed from the far end, it just clears
   * its timer and returns with no error and no reconnect (see the "TODO: check keepalive
   * implementation" comment left in ssh2's own client.js -- this isn't a made-up
   * scenario, it reproduced in production after a fleet-wide reboot). Doesn't wait for
   * or rely on 'close' ever firing on the stale client, unlike a plain client.end() --
   * this exists specifically because that event was confirmed not to arrive here. */
  forceReconnect() {
    ...
    if (staleClient) {
      try {
        ...
        // Already broken -- that's the whole reason this method exists.
      }
      // If the old client's 'close'/'error' does eventually fire (just late), its
      // listeners are gone -- connectOnce() below already owns reconnecting now, and a
      // second trigger from the stale client would just race it.
  ...
  bridgeChannel(channel) {
    ...
    const onData = (chunk) => {
      ...
      if (handshake.shell) {
        this.openShellSession(channel, rest);
        return;
      }
      ...
      if (handshake.detectBaud) {
        this.sendBaudDetect(channel, handshake.path);
        return;
      }
      ...
      if (handshake.upsStatus) {
        this.sendUpsStatus(channel);
        return;
      }
  ...
  /** Bridges a tunneled channel to a real interactive shell on this box, running as this
   * app's own unprivileged service account -- no sudo, no root. Off by default (see
   * configStore's remoteShell.enabled); an admin has to deliberately turn it on in the
   * Fleet panel before a hub operator can ever reach this.
   *
   * Spawned through `script` (util-linux, present on every Debian-family box this
   * targets) rather than a plain child_process pipe, so the shell gets a REAL pty --
   * job control, Ctrl-C, line editing, and interactive prompts all behave the way an
   * operator expects from "a terminal," not a raw byte pipe. This avoids pulling in a
   * native npm addon (node-pty) just to get the same thing, which would need a working
   * native toolchain at every future `npm install` on every box, including inside a
   * QEMU-emulated first boot -- one more thing to break, for no benefit `script` doesn't
   * already give for free. Falls back to a plain (non-pty) shell if `script` isn't
   * present, rather than refusing outright -- ping/traceroute/etc still work fine
   * without a pty, just without the interactive niceties. */
  /** Spawns `script` for a real pty; if that binary is missing (ENOENT, reported async via
   * the 'error' event -- spawn() itself never throws for this), falls back to a plain
   * bash with no pty. The 'spawn' event (fired only once the OS exec actually succeeded)
   * is what actually distinguishes those two outcomes -- child.pid alone isn't reliable
   * for this, since it can already be set before an async exec failure is reported. */
  spawnShellChild() {
    return new Promise((resolve) => {
      const child = spawn('script', ['-qfc', '/bin/bash -l', '/dev/null'], { stdio: 'pipe' });
      let settled = false;
      child.once('spawn', () => {
        if (settled) return;
        settled = true;
      ...
      child.once('error', () => {
        if (settled) return;
        settled = true;
        resolve(spawn('/bin/bash', ['-l'], { stdio: 'pipe' }));
      });
    });
  }

  async openShellSession(channel, leftoverData) {
    if (!configStore.getConfig().remoteShell.enabled) {
      channel.end('remote shell is disabled on this box -- enable it in the Fleet panel first\n');
      return;
    }

    const child = await this.spawnShellChild();
    if (channel.destroyed) {
      child.kill();
      return;
    }

    this.log('fleet remote-shell tunnel session connected');
    if (leftoverData && leftoverData.length) child.stdin.write(leftoverData);
    ...
    const finish = () => {
      ...
      this.log('fleet remote-shell tunnel session disconnected');
    };
  ...
  /** One-shot request/response: probes a device's baud rate/framing (see baudDetect.js) so a
   * hub operator adding a port can auto-detect it instead of guessing. Takes several seconds. */
  sendBaudDetect(channel, devicePath) {
    baudDetect
      .detectForDevice(String(devicePath || ''))
      .then((result) => channel.end(JSON.stringify({ ok: true, ...result })))
      .catch((err) => channel.end(JSON.stringify({ ok: false, error: err.message })));
  }
  ...
  /** One-shot request/response: a fresh live UPS query, gathered on request rather than
   * relying on whatever the last 30s heartbeat happened to carry -- for a hub operator
   * checking right now, not waiting out the polling cadence. */
  sendUpsStatus(channel) {
    upsMonitor
      .getFullStatus()
      .then((status) => channel.end(`${JSON.stringify({ ok: true, ...status })}\n`))
      .catch((err) => channel.end(`${JSON.stringify({ ok: false, error: err.message })}\n`));
  }
