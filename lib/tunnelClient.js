...
const { validateForwardTarget, isLoopbackAddress } = require('./forwardTarget');
...
class TunnelClient extends EventEmitter {
  ...
  bridgeChannel(channel) {
    ...
    const onData = (chunk) => {
      ...
      if (handshake.forward) {
        this.openForwardSession(channel, handshake, rest);
        return;
      }
  ...
  /** Opens a TCP connection from this box to a device on its own network on the hub's behalf
   * (a "forward"), then bridges the tunnelled channel to it. The hub authenticates the person
   * asking; this side enforces what it will connect to (never loopback), and can be turned off
   * entirely with fleet.allowForwarding. Speaks a one-line status first -- "ok" or "err <why>"
   * -- so the hub can tell a refused/unreachable device from an established connection before
   * it tells the SSH client anything. {probe: true} only tests reachability. */
  openForwardSession(channel, handshake, leftoverData) {
    const host = String(handshake.host || '');
    const port = Number(handshake.port);
    const where = `${host}:${port}`;
    const refuse = (msg) => {
      this.log(`refused hub forward to ${where}: ${msg}`);
      channel.end(`err ${msg}\n`);
    };
    if (configStore.getConfig().fleet.allowForwarding === false) {
      return refuse('this box does not allow the hub to forward connections to its network');
    }
    ...
    const finish = () => {
      ...
      sock.destroy();
      if (!channel.destroyed) channel.end();
      if (connected && !handshake.probe) this.log(`hub forward to ${where} closed`);
    };

    sock.once('timeout', () => {
      if (!connected) {
        sock.destroy();
        refuse('connection timed out');
      }
    });
    sock.once('error', (e) => {
      if (!connected) {
        refuse(e.code === 'ECONNREFUSED' ? 'connection refused' : e.code === 'EHOSTUNREACH' ? 'host unreachable' : e.message);
      } else {
        finish();
      }
    });
    sock.once('connect', () => {
      ...
      if (isLoopbackAddress(sock.remoteAddress)) {
        sock.destroy();
        return refuse('loopback destinations are not allowed');
      }
      if (handshake.probe) {
        done = true;
        sock.destroy();
      ...
      this.log(`hub forward opened to ${where}`);
      channel.write('ok\n');
      if (leftoverData && leftoverData.length) sock.write(leftoverData);
      sock.on('data', (d) => {
        if (!channel.destroyed) channel.write(d);
      });
      channel.on('data', (d) => {
