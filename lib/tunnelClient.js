...
class TunnelClient extends EventEmitter {
  ...
  connectOnce() {
    ...
    client.connect({
      ...
      readyTimeout: 10000,
      // Trust-on-first-use, the same model ~/.ssh/known_hosts uses: a blank fingerprint
      // only ever means "no connection has actually happened yet". The very first
      // successful connection pins whatever key it saw -- from that moment on this is
      // no longer optional, every later connection is compared as parsed key bytes
      // (the same technique the hub's own tunnelServer.js uses to match a site by its
      // public key, not a string/text comparison) and a mismatch is rejected outright.
      // An admin who wants zero TOFU window can still paste the hub's fingerprint in
      // by hand before ever enrolling, which skips straight to the enforced state.
