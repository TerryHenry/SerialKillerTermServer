...
class WebTerminalManager extends EventEmitter {
  ...
  attach(httpsServer, sessionMiddleware) {
    ...
    httpsServer.on('upgrade', (req, socket, head) => {
      ...
      // to this endpoint using the victim's own cookies and drive a live serial console
      // through it. Browsers always send Origin on a WebSocket handshake, so requiring it
      // to match this server's own host closes that the same way the CSRF token closes it
      // for HTTP requests.
    ...
    wss.on('connection', async (ws, req) => {
      ...
      const capture = sessionCapture.startCapture(portConfig, {
        username: consoleUser.username,
        method: 'https',
        userCaptureEnabled: !!consoleUser.captureEnabled
      });
