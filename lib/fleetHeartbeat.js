...
class FleetHeartbeat extends EventEmitter {
  ...
  async tick() {
    ...
    if (command === 'apply-update') {
      ...
      this.log('hub pushed a user sync -- replacing hub-managed local logins');
      try {
        const { applied, skipped } = configStore.replaceHubManagedUsers(commandPayload);
        this.log(
          `hub-managed local logins now match the hub's (${applied.length} account${applied.length === 1 ? '' : 's'})${
            skipped.length ? `; skipped ${skipped.join(', ')} because a local account already uses that name` : ''
          }`
        );
      } catch (err) {
