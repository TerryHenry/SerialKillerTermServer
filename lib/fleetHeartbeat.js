...
// Consecutive failed tunneled attempts (each ~30s apart) before assuming the tunnel is
// stuck -- reporting connected but not actually working -- and forcing tunnelClient to
// tear down and reconnect. See tunnelClient.js's forceReconnect() for why this can't just
// wait on the tunnel to notice and recover on its own.
const TUNNEL_FAILURE_RECONNECT_THRESHOLD = 3;
...
class FleetHeartbeat extends EventEmitter {
  ...
  async tick() {
    ...
    const ups = await upsMonitor.getHeartbeatSnapshot();
    const payload = JSON.stringify({
      ...
      // Read-only, unlike the two above -- deliberately not part of the hub-pushed
      // "set local access" command below, since remote shell is meant to require this
      // box's own admin to opt in locally, not something a hub operator can flip on
      // for themselves. Just lets the hub's UI show whether the button will work before
      // someone clicks it and gets a disabled error.
      remoteShellEnabled: config.remoteShell.enabled,
      // null when UPS monitoring is off, not configured, or the live query just failed
      // this cycle (see upsMonitor.getHeartbeatSnapshot) -- the hub treats that the same
      // as "no UPS data available" either way, not as an error state of its own.
      ups,
      tftp: {
    ...
    if (command === 'apply-update') {
      ...
      this.log('hub pushed a user sync -- replacing all local users');
      try {
        const { applied, removed } = configStore.replaceUsers(commandPayload);
        this.log(
          `local users now match the hub's (${applied.length} account${applied.length === 1 ? '' : 's'})${
            removed.length ? `; removed ${removed.length} not on the hub: ${removed.join(', ')}` : ''
          }`
        );
      } catch (err) {
        this.log(`user sync failed, local users unchanged: ${err.message}`);
    ...
    } else if (command === 'set-lldp') {
      const { enabled, cdp, fdp } = commandPayload || {};
      this.log(`hub pushed neighbor-discovery settings -- LLDP ${enabled ? 'enabled' : 'disabled'} (CDP ${cdp ? 'on' : 'off'}, FDP ${fdp ? 'on' : 'off'})`);
      try {
        await lldp.setConfig({ enabled: !!enabled, cdp: !!cdp, fdp: !!fdp });
      } catch (err) {
        this.log(`applying hub-pushed neighbor-discovery settings failed: ${err.message}`);
      }
  ...
  // this preferring the tunnel first. port is null when an admin has left hubApiPort
  // blank (this box relies on the tunnel alone, no fallback address at all) -- in
  // that case a failed tunnel attempt throws instead of dialing a nonexistent port.
  async postJson(host, port, urlPath, body, hubTlsFingerprint) {
    if (tunnelClient.status().connected) {
      try {
        ...
        const result = await this.postJsonOverTunnel(stream, urlPath, body);
        this.tunnelFailureStreak = 0;
        return result;
      } catch (err) {
        // tunnelClient.status().connected can be stale -- true even though the
        // connection has actually stopped working (a confirmed ssh2 bug, see
        // forceReconnect()'s own comment). A one-off failure is left alone (could just be
        // a slow hub or a genuinely transient blip), but repeated ones mean the tunnel
        // needs to be torn down and rebuilt, not politely waited on.
        this.tunnelFailureStreak += 1;
        if (this.tunnelFailureStreak >= TUNNEL_FAILURE_RECONNECT_THRESHOLD) {
          this.log(
            `tunnel reports connected but ${this.tunnelFailureStreak} heartbeats in a row failed over it -- forcing a fresh connection`
          );
          tunnelClient.forceReconnect();
          this.tunnelFailureStreak = 0;
        }
        if (port === null) throw new Error(`tunneled request failed (${err.message}) and no API port is configured to fall back to`);
        this.log(`tunneled request failed (${err.message}) -- falling back to a direct connection`);
      }
    } else if (port === null) {
      throw new Error('tunnel is not connected and no API port is configured to fall back to');
    }
