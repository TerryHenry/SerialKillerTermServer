...
class FleetHeartbeat extends EventEmitter {
  ...
  verifyHubCommand(response, hubHostKeyFingerprint) {
    ...
    if (!hubHostKeyFingerprint) {
      this.log(
        'hub sent a command but this box has not pinned the hub\'s host key -- refusing to act on it (pin the hub\'s fingerprint, from the hub\'s own Account tab, in this box\'s Central Office panel to enable hub-pushed commands)'
      );
      return { command: null, payload: null };
    }
    try {
