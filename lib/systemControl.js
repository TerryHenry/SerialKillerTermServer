...
async function setTimezone(timezone) {
  ...
  // process.env.TZ directly makes every Date call (log timestamps, session
  // "connected at" times, etc.) pick up the change right away, no restart required.
...
async function setOsPassword(password) {
  await runHelper(['os-password-set'], password);
}

async function reboot() {
  await runHelper(['reboot']);
}

async function setHostname(hostname, updateMdns) {
  await runHelper(['hostname-set', hostname, updateMdns ? '1' : '0']);
}

/** Restarts just the app itself (not the whole Pi) -- e.g. to pick up a new TLS cert or a factory-reset config. */
