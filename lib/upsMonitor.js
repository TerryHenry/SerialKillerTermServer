...
const SUPPORTED_DRIVERS = ['usbhid-ups', 'blazer_ser', 'blazer_usb', 'genericups', 'snmp-ups', 'powerman-pdu', 'dummy-ups'];
...
async function setConfig({ enabled, name, driver, port, community, identifier }) {
  ...
  // (there's no separate username in SNMPv1), and powerman-pdu's identifier names a PDU
  // node powerman itself already knows about, not a credential at all.
  const extra = cleanDriver === 'snmp-ups' ? (cleanCommunity || 'public') : cleanDriver === 'powerman-pdu' ? cleanIdentifier : '';
  if (enabled) {
    ...
    if (cleanDriver === 'powerman-pdu' && !cleanIdentifier) {
      throw new Error('a PDU identifier is required for powerman-pdu (the node name as configured in powerman.conf)');
    }
    await ensureInstalled();
    await systemHelper.runHelper(['ups-configure', cleanName, cleanDriver, cleanPort, extra], undefined, 30000);
  } else {
